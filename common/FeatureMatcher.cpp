...
FeatureMatcher::MatchKeypointsInImages(const cv::Mat &input_image_a,
                                       const cv::Mat &input_image_b) {
  std::vector<std::vector<cv::DMatch>> knn_matches;
  const auto input_image_a_descriptors =
      feature_extractor_.GetORBFeaturesFromImage(input_image_a);
  const auto input_image_b_descriptors =
      feature_extractor_.GetORBFeaturesFromImage(input_image_b);

  std::vector<cv::DMatch> good_matches;

  if (!input_image_a_descriptors.empty() &&
      !input_image_b_descriptors.empty()) {
    const int k = 2;
    feature_matcher_.knnMatch(input_image_a_descriptors,
                              input_image_b_descriptors, knn_matches, k);
